...
## Step 2: Provide input to render PDF player
...
| `pkgVersion` | It is `number` to represent the version of the current packages | Optional | Defalut compatibilityLevel `"1.0"` will set | its an optional field. The default is `1.0`.|
 
...
| `sid` | It is an `string` containing user session id. | ```sid = uid  ```|Optional|
| `did` | It is an `string` containing unique device id.| ```fingerPrintjs2```|Optional|
| `uid` |It is an `string` containing user id.| ```actor.id = did ? did : "anonymous" ```|Optional|
| `authToken` | It is an `string` to send telemetry to given endpoint (API uses for authentication) | ```''```|Optional|
